...
the same one embedded into `conflated.parquet`'s own CycloneDX
provenance BOM (not the release image's SBOM described in
[`SUPPLY_CHAIN_SECURITY.md`](SUPPLY_CHAIN_SECURITY.md) — a different
document about a different artifact; see
[`src/provenance.rs`](../src/provenance.rs) for this one), so a given
run's log and its data output can always be tied back together. This
happens regardless of whether the run succeeded — a failed run's log is
