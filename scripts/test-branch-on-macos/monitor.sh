#!/usr/bin/env bash
# Monitors vm_stat and osmdiffs's own RSS, appending a log entry every
...
# one-shot PID goes stale as soon as osmdiffs isn't running yet when
...
while true; do
    printf '=== %s UTC ===\n' "$(date -u +"%Y-%m-%d %H:%M:%S")"
    pid=$(pgrep -f 'osmdiffs run' | head -1)
    if [ -n "$pid" ]; then
        ps -o pid,%cpu,%mem,rss,vsz -p "$pid"
    else
        echo "osmdiffs run: not currently running"
    fi
    vm_stat
    sleep 5
done
